...
from src.datautils.image.rescale_transform import rescale_bigger_pad_smaller_image
from src.datautils.text.charset_token import CharsetToken


def generate_all_char(font, tag):
    # Generate char label
    img = Image.new("RGB", (1664, 140))
    draw = ImageDraw.Draw(img)
    font = ImageFont.truetype(font, 60)
    draw.text((42, 42), "./ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789", fill=(255, 255, 255), font=font)
    # draw.text((40,40), "stack overflow!", fill=(255,0 , 0), font=font)
    img.save('C:/Users/simcor/dev/logs/label_' + tag + '.png')
...
def generate_safran_char(label, font, height, width, path):
    ...
    # font_size = random.randint(32, 64)
    ...
    # coordinate (x, y)
    draw.text((3 + offset, -4), label, font=font, fill=ink_color_bright)

    # img.save('C:/Users/simcor/dev/logs/img_char.png')
    # img.save(path)
    ...
    # img.save('C:/Users/simcor/dev/logs/img_shadow.png')
    # img.save(path)
    ...
    if np.random.rand() < 0.4:
        ...
        img = img.transform((new_width, real_height), Image.AFFINE, (1, m, -xshift if m > 0 else 0, 0, 1, 0), Image.BICUBIC)
        img_shadow = img_shadow.transform((new_width, real_height), Image.AFFINE, (1, m, -xshift if m > 0 else 0, 0, 1, 0), Image.BICUBIC)

    #img.save('C:/Users/simcor/dev/logs/shear.png')
    ...
    # max_value = torch.max(char_tensor)
    # print("Max char tensor: " + str(max_value))
    ...
    img_background = gaussian_blur(img_background, kernel_size=[kx, ky], sigma=(0.1, 5.))  # V5 kernel_size=(3, 7), sigma=(0.3, 4.))
    ...
    merge *= 255
    # max_value = torch.max(merge)
    # print("Max before rescale: " + str(max_value))
    ...
    # max_value = torch.max(merge)
    # print("Max end: " + str(max_value))
    return merge


def generate_n_sample_v5(dir_save, char_label, fonts, nb_generate_per_class, height, width, path_save_db):
...
if __name__ == '__main__':
    ...
    fonts = [font_leddotmatrix, font_enhanceddotdigit]

    # # Compare several fonts
    # generate_all_char(font_leddotmatrix, "leddotmatrix")
    # generate_all_char(font_enhanceddotdigit, "enhanceddotdigit ")

    charset_file = "../../../data/charset_dageec_task2.txt"
    charset = CharsetToken(charset_file)
    char_list = charset.get_charset_list()
    char_dict = charset.get_charset_dictionary()

    dir_save = "C:/Users/simcor/dev/data/Safran-MNIST_DLS/synthetic_n4000/"
    n_train = 4000
    ...
    generate_n_sample_v5(dir_train, char_list, fonts, 100, height, width, path_save_db_train)
    # # Test
    ...
    #
    # generate_n_sample_v5(dir_test, char_list, fonts, 4, height, width, path_save_db_test)
    #
    # # # All
    # # Train
    ...
    #
    # generate_n_sample_v5(dir_train, char_list, fonts, n_train, height, width, path_save_db_train)
    # # Test
    ...
    #
    # generate_n_sample_v5(dir_test, char_list, fonts, 1600, height, width, path_save_db_test)
