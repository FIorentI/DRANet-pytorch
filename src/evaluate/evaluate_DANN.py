...
from src.models.model_utils import load_pretrained_model, load_pretrained_DANN_combined
import model
...
parser.add_argument("--path_encoder", default="", help="path of pretrained encoder", type=str)
parser.add_argument("--path_classifier", default="", help="path of pretrained classifier", type=str)
...
if args.cnn_model == CNNModel.ResNet18:
    combined_model = ResNetTorch(BasicBlock, [2, 2, 2, 2], nb_channel_input=nb_channel_input, num_classes=nb_char_all)
if args.cnn_model == CNNModel.ResNet34:
    combined_model = ResNetTorch(BasicBlock, [3, 4, 6, 3], nb_channel_input=nb_channel_input, num_classes=nb_char_all)
if args.cnn_model == CNNModel.ResNet50:
    combined_model = ResNetTorch(Bottleneck, [3, 4, 6, 3], nb_channel_input=nb_channel_input, num_classes=nb_char_all)

#combined_model = DANNModel(model)

# 3. Créer les sous-modèles Extractor et Classifier
encoder = model.Extractor(combined_model).to(device)
classifier = model.Classifier(combined_model).to(device)

# 4. Charger les poids sauvegardés dans chaque sous-modèle
encoder.load_state_dict(torch.load(args.path_encoder, map_location=torch.device(device)))
classifier.load_state_dict(torch.load(args.path_classifier, map_location=torch.device(device)))

model = model.DANNModel(encoder, classifier)
